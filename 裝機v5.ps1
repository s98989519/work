& {
    ...
    while ($pChoice -notmatch "^[A-C]$") {
        ...
        if ($pChoice -notmatch "^[A-C]$") { 
            Write-Host "[ERROR] 輸入錯誤，只能輸入 A, B 或 C" -ForegroundColor Red 
        }
    }
    $selectedFile = $Printer_Table[$pChoice].File
    Write-Host "[OK] 已選取地區: $($Printer_Table[$pChoice].Name)" -ForegroundColor Green
    ...
    while ($choices.Count -eq 0) {
        ...
        if ($choices.Count -eq 0) { 
            Write-Host "[ERROR] 輸入格式錯誤，請重新輸入編號 1-5" -ForegroundColor Red 
        }
    }
    ...
    # --- 4. 檔案佈署與印表機安裝 ---
    Write-Host "`n--- 執行檔案佈署與印表機安裝 ---" -ForegroundColor Cyan
    ...
    if (Test-Path $sourceDir) {
        ...
        # 處理所選地區的印表機檔案
        $targetPath = Join-Path $printerDestDir $selectedFile
        $finalVbs = Join-Path $printerDestDir "00GPSfollowme.vbs"
        
        if (Test-Path $targetPath) {
            if (Test-Path $finalVbs) { Remove-Item $finalVbs -Force }
            Rename-Item -Path $targetPath -NewName "00GPSfollowme.vbs"
            Write-Host "正在執行印表機腳本: $selectedFile ..." -ForegroundColor Yellow
            Start-Process "wscript.exe" -ArgumentList "`"$finalVbs`""
        }
    }

    # --- 5. 視窗啟動 ---
    Write-Host "--- 啟動 BitLocker PIN 設定 ---" -ForegroundColor Yellow
    $mbam = "C:\Program Files\Microsoft\MDOP MBAM\MBAMControlUI.exe"
    if (Test-Path $mbam) { Start-Process $mbam }

    Write-Host "--- 開啟 Edge 設定網頁 ---" -ForegroundColor Cyan
